...
export const createIndexHtml = async function (
  ...
  if (mode === 'dev') {
    ...
    const createClientOverlay = (error, title) => {
      ...
      document.querySelector("#lyr-cli-client-overlay").contentWindow.document.body.innerHTML = \`
              <div style="font-size: 20px;color: red;margin-bottom: 20px;">$\{title\}</div>
              ...
              <pre style="color: red; margin: 10px 0">$\{error\}</pre>
            \`;
      ...
      if ('WebSocket' in window) {
        ...
        ws.onmessage = (message) => {
          if (message.data === "1") {
            ...
            createClientOverlay(message.data, '编译异常');
          }
        };
      }
    };
    window.addEventListener('error', function (e) {
      createClientOverlay(e.message, "捕获异常")
    })
</script>`;
  }
